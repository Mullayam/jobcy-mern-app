...
class Authentication {
    ...
    async ForgetPassword(req: Request, res: Response): Promise<Response | void> {
        try {
            ...
            // check user exist or not
            const isUser = await this.CheckUserExistorNot(req.body.email)
            if (!isUser) {
            ...
            return JSONResponse.Response(req, res, "Email Sent", {}, 200)
    ...
    async RequestNewVerificationCode(req: Request, res: Response): Promise<void> {
        try {
            ...
            const GeneratedToken = Helpers.generateToken()
            ...
            const PrepareTemplate = EmailVerification(process.env.APP_IMG_URL as string, GeneratedToken)
